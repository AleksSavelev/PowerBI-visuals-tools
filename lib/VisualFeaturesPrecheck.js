...
class FeatureAnalyzer {
    ...
    eslintCheck() {
        ...
        const eslintrcPath = `"${__dirname}/.CVeslintrc.js"`;
        const visualPath = `"${process.cwd()}/src"`;
        ConsoleWriter.info("Running eslint check...");
        try {
            childProcess.execSync(`npx eslint ${visualPath} --config ${eslintrcPath} --resolve-plugins-relative-to ${rootPath}`);
        } catch (error) {
            ConsoleWriter.error(error.stdout.toString());
        }
